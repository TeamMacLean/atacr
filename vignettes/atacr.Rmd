...
* mean_coverage `<numeric double>` (0..Inf) average number of reads mapping over each position in the bait
* breadth `<numeric double>` (0..100) Percent of positions in the base covered with reads
* read_count `<numeric integer>` (0..Inf) Number of reads mapping to the bait region
...
* `mean_coverage` _becomes_ `sample_bait_mean_coverage`
* `read_count` _becomes_ `sample_bait_read_count`
* `breadth` _becomes_ `sample_bait_breadth`
...
You can find a summary of the negative controls you found in each sample with `count_negatives_found()` and mean coverage of those negative controls with `negative_control_mean_coverage()`

```{r}
dplyr::left_join(atacr::count_negatives_found(norm_neg), atacr::negative_control_mean_coverage(norm_neg), by="sample")
```

You can get the mean coverage of the negative controls with `negative_control_mean_coverage(norm_neg)`
```{r}
atacr::negative_control_mean_coverage(norm_neg)
```
