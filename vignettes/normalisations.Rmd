...
This option allows you to perform a scaling of the data based on user-specified control regions, usually these will be genomic windows corresponding to baits from control genes/regions. A one-step option is to provide these control gene locations to `control_window_normalise()` in a separate file.

```{r, eval=FALSE}
control_gene_normalised_counts <- control_window_normalise(counts, "my_controls.csv")
```
...
## Comparing sets of potential control genes
